...
public class CommandListener extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent e) {
        ...
        if (e.getMessage().getContentRaw().startsWith(codes.dev_up.DevUpBot.config.getPreferences().node("commands").get("PREFIX", "!")) && !e.getMessage().getAuthor().getId().equals(e.getJDA().getSelfUser().getId())) {
            try {
                handleCommand(codes.dev_up.DevUpBot.parser.parse(e.getMessage().getContentRaw(), e));
                addToLogfile(e);
